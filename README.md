...
Depending on the usage for this classification, there are cases to be made on whether a bagged approach vs RUSBoosted approach is more appropriate. If we require the sample to have a low number of blebbed spermatazoa then an algorithm which has a higher true positive rate for blebbed spermatazoa could be preferable to one with a higher total accuracy. We also note that the top three predictors flagellum length, beat period and $\beta$ were poor predictors for blebbed spermatazoa, suggesting that $\alpha$ and MDC are important to determining whether a sample is blebbed or unblebbed. Another approach in order to reduce the misclassification for blebbed could be to introduce a prior based on what the blebbed percentage might be for a given sample.
